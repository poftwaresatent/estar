...
#ifdef DEBUG
# define ESTAR_FOO_KERNEL_DEBUG
#else
# undef ESTAR_FOO_KERNEL_DEBUG
#endif

#ifdef ESTAR_FOO_KERNEL_DEBUG
...
namespace estar {
  ...
  DoCompute(Propagator & propagator) const
  {
    ...
    const double tmax(v1 + 2 * scale * meta);
    debugstream dbg;
    ...
    const double tnonfb(v1 + (1+meta*meta) * (2*scale+v2-v1) / (2+meta));
